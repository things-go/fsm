...
type Transform[E constraints.Ordered, S constraints.Ordered] struct {
	// Event is the event used when calling for the transition.
	Event E
...
type Translator[E constraints.Ordered, S constraints.Ordered] struct {
	// contain all support event.
	events map[E]struct{}
	// trans map events and source states to destination states.
	trans map[eKey[E, S]]S
}

func NewTranslator[E constraints.Ordered, S constraints.Ordered](transitions []Transform[E, S]) Translator[E, S] {
	ts := Translator[E, S]{
...
// Trans return Trans
func (ts *Translator[E, S]) Trans() map[eKey[E, S]]S {
	return ts.trans
}
